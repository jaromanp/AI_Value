...
def read_csv():
    #Leyendo archivo
    urlden = "https://aivalue.s3.amazonaws.com/data/InterpolatedDenMonth.csv"
    urlnum =  "https://aivalue.s3.amazonaws.com/data/InterpolatedNumMonth.csv"
    df = pd.read_csv(urlden)
    df_N = pd.read_csv(urlnum)
...
def apply_function(individual):
    ...
    urlmodelnum = 'https://aivalue.s3.amazonaws.com/data/modelnum.json'
    urlmodelnumw = 'https://aivalue.s3.amazonaws.com/data/modelnum.h5'
    urlmodelden = 'https://aivalue.s3.amazonaws.com/data/modelden.json'
    urlmodeldenw = 'https://aivalue.s3.amazonaws.com/data/modelden.h5'
    json_file = open(urlmodelnum, 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model_num = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model_num.load_weights(urlmodelnumw)
    # Denominador
    json_file = open(urlmodelden, 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model_den = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model_den.load_weights(urlmodeldenw)
    w1 = individual["w1"]
    w2 = individual["w2"]
    w3 = individual["w3"]
    w4 = individual["w4"]
...
def predict():
    ...
    if input.empty:
        ...
        responses = jsonify(best_individual.to_json(orient="records"))
